...
const createTaskForm = (e) => {
  ...
  _contentDiv.append(
    _createChoiceContainer('Project', _createProjectDropdown, task)
  );
  _contentDiv.append(
    _createChoiceContainer('High priority?', _createPriorityToggle, task)
  );
  _contentDiv.append(
    _createChoiceContainer('Due date?', _createDueDateOptions, task)
  );
  _contentDiv.append(_createChoiceContainer('Notes:', _createNotesBox, task));
  _contentDiv.append(_createDateCompleted(task));
  const allCheckboxes = document.querySelectorAll('.checkbox-container');
  allCheckboxes.forEach((checkbox) =>
    checkbox.classList.add('tf-checkbox-container')
  );
};
...
const _createChoiceContainer = (text, func, task) => {
  ...
  choiceContainer.append(_createChoiceLabel(text), func(task));
...
const _createProjectDropdown = (task) => {
  ...
  projectDropdown.addEventListener('change', (e) => {
    const newProj = model.projectArray.find(
...
const _createPriorityToggle = (task) => {
  ...
  const checkboxContainer = view.createCheckbox(
    task,
    task.isHighPriority,
    _togglePriority
  );
...
const _toggleDueDate = (e) => {
  ...
  if (task.dueDate) {
    ...
    controller.changeProperty(
      task,
      'dueDate',
      new Date(yearMonthDay[0], yearMonthDay[1] - 1, yearMonthDay[2])
    );
...
const _createDueDatePicker = (task) => {
  ...
  if (task.dueDate) {
    picker.value = format(
      task.dueDate.seconds ? task.dueDate.toDate() : task.dueDate,
      'yyyy-MM-dd'
    );
  } else {
...
const _createDateCompleted = (task) => {
  ...
  if (task.completionDateTime) {
    completionStamp.textContent = `Completed on ${format(
